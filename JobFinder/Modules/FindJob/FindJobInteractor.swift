...
class FindJobInteractor: FindJobInteractorProtocol {
    
    weak var presenter: FindJobPresenterProtocol?
    
    func searchJob(position: String, lat: Double?, lng: Double?) {
        let provider = DataManager.shared.selectedProvider!
        let params = provider.getParams(lat: lat, lng: lng, position: position)
